...
1.将Entity文件夹和代码导入SpringBoot文件【如工具类，不要放到测试类中，最好同一个包下】，需要引入：Lombok、Mysql相关依赖

2.检查主键是否有@TableId注解，含@TableField（exist=false）注解字段会被过滤

3.默认引擎：InnoDB、AUTO_INCREMENT = 1、CHARACTER SET = utf8、COLLATE = utf8_general_ci、Mysql版本：8
